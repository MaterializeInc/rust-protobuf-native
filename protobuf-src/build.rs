...
fn main() -> Result<(), Box<dyn Error>> {
    // Check out the protobuf source code
    let build_dir = PathBuf::from(env::var_os("OUT_DIR").unwrap());
    println!("Building in directory {:?}", build_dir);
    assert!(Command::new("git")
            .current_dir(build_dir.clone())
            .arg("clone")
            .arg("--depth=1")
            .arg("--branch=v27.2")
            .arg("https://github.com/protocolbuffers/protobuf")
            .status()
            .unwrap()
            .success());
    assert!(Command::new("git")
            .current_dir(build_dir.join("protobuf").join("third_party"))
            .arg("clone")
            .arg("--depth=1")
    ...
    let install_dir = cmake::Config::new(build_dir.join("protobuf"))
